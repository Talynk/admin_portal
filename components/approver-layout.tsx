...
export function ApproverLayout({ children }: { children: React.ReactNode }) {
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                <Avatar className="h-8 w-8">
                  <AvatarImage src="/placeholder.svg?height=32&width=32" alt={approver?.first_name || "Approver"} />
                  <AvatarFallback>
                    {approver?.first_name?.charAt(0) || approver?.email?.charAt(0) || "A"}
                  </AvatarFallback>
                </Avatar>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-56" align="end" forceMount>
              <DropdownMenuLabel className="font-normal">
                <div className="flex flex-col space-y-1">
                  ...
                  <p className="text-xs leading-none text-muted-foreground">{approver?.email}</p>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
